...
export default function Presentation() {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <header className="flex items-center justify-between gap-3 px-4 sm:px-10 py-3.5 sm:py-4 border-b border-line bg-bg-side/60 backdrop-blur-sm shrink-0">
        <div className="flex items-baseline gap-3 min-w-0">
          ...
          <span className="hidden sm:block text-ink-700 mono text-xs">/</span>
          <span className="hidden sm:block mono text-xs uppercase tracking-mono text-ink-500 truncate">
...
function SlideStage({ children, image, className = '' }) {
  ...
  return (
    <div className={`min-h-full px-5 sm:px-16 py-10 sm:py-16 relative ${className}`}>
      {isBackground && (
        <>
...
function CoverSlide({ slide }) {
  return (
    <SlideStage image={slide.image}>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-6 sm:py-10">
        <div className="kicker mb-6 sm:mb-10">{slide.kicker}</div>
        <h1 className="font-display text-5xl sm:text-9xl font-black leading-[0.95] sm:leading-[0.9] tracking-tight text-ink-100">
          {slide.title}
        </h1>
        {slide.subtitle && (
          <p className="mt-5 sm:mt-8 text-2xl sm:text-5xl text-ink-300 font-light leading-tight">
            {slide.subtitle}
          </p>
        )}
        {slide.meta?.length > 0 && (
          <div className="mt-10 sm:mt-16 flex flex-wrap items-center gap-x-3 gap-y-2 sm:gap-5 text-sm sm:text-base mono text-ink-500 uppercase tracking-mono">
            {slide.meta.map((m, i) => (
              <span key={i} className="flex items-center gap-x-3 sm:gap-5">
    ...
    <SlideStage image={slide.image?.position === 'background' ? slide.image : null}>
      <div className={`grid ${hasImage ? 'lg:grid-cols-2 gap-12' : ''} items-center min-h-[calc(100vh-220px)] py-10`}>
        <div>
          ...
          {slide.pullquote && (
            <div className="mt-8 pr-4 sm:pr-5 border-r-2 border-brand max-w-2xl">
              <p className="text-xl sm:text-2xl text-brand font-medium italic leading-snug">{slide.pullquote}</p>
            </div>
          )}
          {hasLogos && (
            <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-5 max-w-2xl">
              {slide.logos.map((logo, i) => (
                <div key={i} className="card-elev p-6 grid place-items-center hover:border-brand transition aspect-square">
                  <img src={logo.url} alt={logo.name} className="w-12 h-12 object-contain" style={{ filter: 'brightness(0) saturate(100%) invert(58%) sepia(42%) saturate(720%) hue-rotate(338deg) brightness(93%) contrast(88%)' }} />
                </div>
              ))}
            </div>
          )}
      ...
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        ...
        <ul className="grid sm:grid-cols-2 gap-4 sm:gap-5">
          {slide.items.map((item, i) => (
            <li key={i} className="flex items-start gap-4 sm:gap-5 card-elev p-5 sm:p-6 border border-line hover:border-brand/40 transition">
              <span className="mono text-brand text-2xl sm:text-3xl font-bold shrink-0 leading-none">
                {String(i + 1).padStart(2, '0')}
              </span>
              <span className="text-lg sm:text-xl text-ink-100 leading-snug">{item}</span>
            </li>
          ))}
        </ul>
      </div>
        ...
        <div className="flex items-center gap-6 mb-4 flex-wrap">
          {slide.logo && (
            <div className="card-elev p-3 grid place-items-center w-16 h-16 shrink-0">
              <img src={slide.logo} alt={slide.title} className="w-10 h-10 object-contain" style={{ filter: 'brightness(0) saturate(100%) invert(58%) sepia(42%) saturate(720%) hue-rotate(338deg) brightness(93%) contrast(88%)' }} />
            </div>
          )}
  ...
  return (
    <SlideStage>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        ...
        {slide.intro && <p className="text-base sm:text-xl text-ink-300 mb-6 sm:mb-8 max-w-3xl">{slide.intro}</p>}
        <div className="card-elev overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="bg-bg-card">
                <th className="mono text-[0.65rem] sm:text-xs uppercase tracking-mono text-brand text-right px-3 py-3 sm:px-6 sm:py-4 border-b border-line">משימה</th>
                <th className="mono text-[0.65rem] sm:text-xs uppercase tracking-mono text-brand text-right px-3 py-3 sm:px-6 sm:py-4 border-b border-line">הכלי המומלץ</th>
                <th className="mono text-[0.65rem] sm:text-xs uppercase tracking-mono text-brand text-right px-3 py-3 sm:px-6 sm:py-4 border-b border-line hidden sm:table-cell">למה</th>
              </tr>
            </thead>
            <tbody>
              {slide.rows.map((r, i) => (
                <tr key={i} className="hover:bg-bg-card/50 transition align-top">
                  <td className="px-3 py-3 sm:px-6 sm:py-4 text-sm sm:text-lg text-ink-100 font-medium border-b border-line">{r.task}</td>
                  <td className="px-3 py-3 sm:px-6 sm:py-4 text-sm sm:text-lg text-brand font-bold border-b border-line">{r.tool}</td>
                  <td className="px-3 py-3 sm:px-6 sm:py-4 text-base text-ink-300 border-b border-line hidden sm:table-cell">{r.why}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="sm:hidden mono text-[0.65rem] text-ink-700 mt-3 tracking-mono">↻ סובב את המכשיר לרוחב לעמודת ׳למה׳</p>
      </div>
    </SlideStage>
  )
}
  ...
  return (
    <SlideStage>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        ...
        {slide.intro && <p className="text-base sm:text-xl text-ink-300 mb-7 sm:mb-10 max-w-3xl">{slide.intro}</p>}
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
          {slide.tools.map((tool, i) => (
            <div key={i} className="card-elev p-5 sm:p-7 hover:border-brand transition flex items-start gap-4 sm:gap-5">
              {tool.logo && (
                <div className="card-elev w-12 h-12 sm:w-14 sm:h-14 grid place-items-center shrink-0">
                  <img src={tool.logo} alt={tool.name} className="w-8 h-8 object-contain" style={{ filter: 'brightness(0) saturate(100%) invert(58%) sepia(42%) saturate(720%) hue-rotate(338deg) brightness(93%) contrast(88%)' }} />
                </div>
              )}
...
function QuoteSlide({ slide }) {
  return (
    <SlideStage image={slide.image}>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        <div className="kicker mb-7">{slide.kicker}</div>
        <h2 className="font-display text-3xl sm:text-5xl font-extrabold leading-tight tracking-tight text-ink-100 mb-8 sm:mb-12">
          {slide.title}
        </h2>
        <div className="relative max-w-4xl">
          <div className="absolute -right-1 -top-8 sm:-right-4 sm:-top-12 text-brand/20 font-display text-[7rem] sm:text-[12rem] leading-none select-none pointer-events-none">"</div>
          <blockquote className="relative pr-5 sm:pr-8 border-r-4 border-brand">
            <p className="text-2xl sm:text-4xl text-ink-100 font-medium leading-snug italic">
              {slide.quote}
            </p>
          </blockquote>
        </div>
        {slide.body && (
          <div className="prose-slide max-w-3xl mt-8 sm:mt-10" dangerouslySetInnerHTML={{ __html: slide.body }} />
        )}
      </div>
    </SlideStage>
  )
                  ...
                  {group.logo && (
                    <div className="card-elev w-12 h-12 grid place-items-center shrink-0 bg-bg-card">
                      <img
                        ...
                        style={{ filter: 'brightness(0) saturate(100%) invert(58%) sepia(42%) saturate(720%) hue-rotate(338deg) brightness(93%) contrast(88%)' }}
                      />
                    </div>
                  )}
                  <div>
          ...
          <ol className="space-y-3 max-w-4xl">
            {slide.items?.map((item, i) => (
              <li key={i} className="card-elev p-5 sm:p-6 flex items-start gap-4 sm:gap-6 hover:border-brand/40 transition">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-lg sm:text-xl font-bold text-ink-100 leading-tight">{item.label}</div>
                  {item.detail && <div className="text-sm sm:text-base text-ink-300 mt-1.5 leading-relaxed">{item.detail}</div>}
                </div>
              </li>
            ))}
          </ol>
...
function RecapSlide({ slide }) {
  return (
    <SlideStage>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        <div className="kicker mb-7">{slide.kicker}</div>
        <h2 className="font-display text-4xl sm:text-6xl font-extrabold leading-tight tracking-tight text-ink-100 mb-8 sm:mb-12">
          {slide.title}
        </h2>
        <ul className="space-y-4 sm:space-y-5 max-w-4xl">
          {slide.points.map((p, i) => (
            <li key={i} className="flex items-start gap-4 sm:gap-6 pb-4 sm:pb-5 border-b border-line">
              <span className="mono text-brand text-2xl sm:text-3xl font-bold shrink-0 leading-none mt-1">
                0{i + 1}
              </span>
              <span className="text-lg sm:text-2xl text-ink-100 leading-snug">{p}</span>
            </li>
          ))}
        </ul>
      </div>
          ...
          {slide.steps.map((step, i) => (
            <li key={i} className="relative">
              <div className="card-elev p-5 sm:p-6 flex items-start gap-4 sm:gap-5 hover:border-brand/40 transition">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-lg sm:text-xl font-bold text-ink-100 leading-tight mb-1.5">{step.label}</div>
                  {step.detail && (
                    <div className="text-sm sm:text-base text-ink-300 leading-relaxed">{step.detail}</div>
                  )}
                </div>
              </div>
...
function NextLessonSlide({ slide }) {
  return (
    <SlideStage image={slide.image}>
      <div className="flex flex-col justify-center min-h-[calc(100vh-220px)] py-10">
        <div className="kicker mb-7">{slide.kicker}</div>
        <div className="card-elev accent-stripe p-6 sm:p-14 max-w-4xl">
          <div className="mono text-xs uppercase tracking-kicker text-ink-500 mb-3 sm:mb-4">
            שיעור {slide.lessonNumber}
          </div>
          <h2 className="font-display text-4xl sm:text-7xl font-black leading-[1.05] sm:leading-none tracking-tight text-ink-100 mb-4 sm:mb-5">
            {slide.title}
          </h2>
          <p className="text-2xl sm:text-3xl text-brand font-light mb-5 sm:mb-8">{slide.subtitle}</p>
          <p className="text-base sm:text-xl text-ink-300 leading-relaxed max-w-3xl">{slide.preview}</p>
        </div>
      </div>
    </SlideStage>
  )
